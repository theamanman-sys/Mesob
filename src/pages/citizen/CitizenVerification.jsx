...
export default function CitizenVerification() {
  ...
  return (
    <div>
      <div className="mb-6">
        <h1 className="text-2xl font-black text-gray-900">{t('Identity Verification')}</h1>
        <p className="text-gray-500 mt-1">{t('Verify your identity to earn the MESOB Verified badge')}</p>
      </div>
          ...
          <div className="flex items-center justify-between flex-wrap gap-4">
            <div>
              <div className="flex items-center gap-2 mb-1">
                ...
                <h2 className="text-lg font-bold">{t('Verification Status')}</h2>
              </div>
              <p className="text-blue-200 text-sm">{t('Verify at least 2 documents to earn the MESOB Verified badge')}</p>
            </div>
            <div className="flex items-center gap-4">
              <div className="text-center"><div className="text-2xl font-bold">{verifiedCount}/{totalDocs}</div><div className="text-xs text-blue-200">{t('Verified')}</div></div>
              <div className="text-center"><div className="text-2xl font-bold">{pendingCount}</div><div className="text-xs text-blue-200">{t('Pending')}</div></div>
              <div className={`px-4 py-2 rounded-xl text-center ${badge.isMesobVerified ? 'bg-yellow-400 text-yellow-900' : 'bg-white/20 text-white'}`}>
                <div className="flex items-center gap-1.5 text-sm font-bold">
                  {badge.isMesobVerified ? <><BadgeCheck className="w-5 h-5" /> {t('MESOB Verified')} ✓</> : <><Shield className="w-4 h-4" /> {t('Not Verified')}</>}
                </div>
              </div>
            </div>
          </div>
          {!badge.isMesobVerified && (
            <div className="mt-4 bg-white/10 rounded-xl p-3">
              <div className="flex items-center gap-2 text-sm">
                ...
                <span>{t('You need')} {2 - verifiedCount} {t('more verified document(s) to get the MESOB Verified badge.')}</span>
              </div>
            ...
            <motion.div key={v.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
              ...
              <div className="flex items-start justify-between mb-3">
                ...
                <span className={`inline-flex items-center gap-1 text-xs px-2.5 py-1 rounded-full font-medium ${STATUS_STYLES[v.status] || 'bg-gray-100 text-gray-600'}`}>
                  <StatusIcon className="w-3 h-3" /> {v.status === 'not_submitted' ? t('Not Submitted') : v.status.charAt(0).toUpperCase() + v.status.slice(1)}
                </span>
              </div>
              ...
              {v.status === 'verified' && v.verifiedAt && (
                <div className="text-xs text-green-600 mb-3 flex items-center gap-1"><CheckCircle className="w-3 h-3" /> {t('Verified')} {new Date(v.verifiedAt).toLocaleDateString()}</div>
              )}
              ...
              {v.status === 'pending' && (
                <div className="flex items-center gap-2 text-xs text-yellow-600 bg-yellow-50 px-3 py-2 rounded-xl">
                  <Clock className="w-3.5 h-3.5" /> {t('Your document is being reviewed by the verification team.')}
                </div>
              )}
            </motion.div>
          )
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 mt-6">
        <h3 className="text-sm font-semibold text-gray-700 mb-3 flex items-center gap-2"><Award className="w-4 h-4 text-yellow-500" /> {t('About MESOB Verification')}</h3>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
          <div className="p-3 bg-blue-50 rounded-xl"><div className="font-semibold text-blue-700 mb-1">{t('Step 1')}</div><div className="text-blue-600">{t('Upload your official documents (National ID, Passport, etc.)')}</div></div>
          <div className="p-3 bg-purple-50 rounded-xl"><div className="font-semibold text-purple-700 mb-1">{t('Step 2')}</div><div className="text-purple-600">{t('Our verification team reviews and confirms your documents')}</div></div>
          <div className="p-3 bg-green-50 rounded-xl"><div className="font-semibold text-green-700 mb-1">{t('Step 3')}</div><div className="text-green-600">{t('Earn the MESOB Verified badge on your profile and join verified citizens')}</div></div>
        </div>
        <div className="mt-4 p-3 bg-gray-50 rounded-xl text-xs text-gray-500">
          <strong>{t('Benefits of verification:')}</strong> {t('Verified users gain access to premium services, faster application processing, and are counted in Ethiopia\'s verified digital population statistics.')}
        </div>
      </motion.div>
    </div>
  )
