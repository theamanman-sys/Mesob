  ...
  return (
    <div>
      <div className="flex items-center gap-3 mb-6">
        ...
        <h1 className="text-2xl font-bold text-gray-900">{t('Insurance')}</h1>
      </div>

      <div className="grid md:grid-cols-2 gap-6">
        <div className="space-y-6">
          <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-200">
            <h2 className="text-lg font-semibold text-gray-800 mb-4">{t('Select Plan')}</h2>
            <div className="grid grid-cols-2 gap-3">
              {plans.map((p) => (
                <button key={p.id} onClick={() => setSelectedPlan(p.id)}
                  ...
                  <span className="text-sm font-medium text-gray-800">{t(p.label)}</span>
                  <span className="text-xs text-gray-400">{(p.rate * 100).toFixed(1)}% {t('rate')}</span>
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-200">
            <h2 className="text-lg font-semibold text-gray-800 mb-4 flex items-center gap-2">
              <Calculator className="w-5 h-5 text-blue-500" /> {t('Coverage Details')}
            </h2>
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">{t('Coverage Amount:')} <span className="font-bold text-blue-600">{format(coverage)} ETB</span></label>
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">{t('Your Age:')} <span className="font-bold text-blue-600">{age}</span></label>
              ...
              <button onClick={calculate}
                ...
                <Calculator className="w-5 h-5" /> {t('Calculate Premium')}
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-6">
          {result && (
            <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-200">
              <h2 className="text-lg font-semibold text-gray-800 mb-4 flex items-center gap-2">
                <CheckCircle className="w-5 h-5 text-green-500" /> {t('Your Premium')}
              </h2>
              <div className="space-y-4">
                <div className="p-4 bg-blue-50 rounded-lg text-center">
                  <p className="text-sm text-gray-500">{t(result.plan)} &middot; {format(result.coverage)} ETB {t('coverage')}</p>
                  <p className="text-3xl font-bold text-blue-600 mt-1">{format(result.monthly)} ETB</p>
                  <p className="text-xs text-gray-400">{t('per month')}</p>
                </div>
                <div className="p-4 bg-green-50 rounded-lg text-center">
                  <p className="text-sm text-gray-500">{t('Annual Premium')}</p>
                ...
                <div className="space-y-2 p-4 bg-gray-50 rounded-lg text-sm">
                  <div className="flex justify-between"><span className="text-gray-500">{t('Coverage')}</span><span className="font-semibold">{format(result.coverage)} ETB</span></div>
                  <div className="flex justify-between"><span className="text-gray-500">{t('Age factor')}</span><span className="font-semibold">{(1 + (result.age - 18) * 0.005).toFixed(2)}x</span></div>
                </div>
                <button className="w-full py-3 bg-green-600 text-white rounded-lg font-semibold hover:bg-green-700 transition flex items-center justify-center gap-2">
                  <CreditCard className="w-5 h-5" /> {t('Pay Premium Now')}
                </button>
                <p className="text-xs text-gray-400 text-center">{t('Secure payment via trusted gateway')}</p>
              </div>
            </div>
          )}

          <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-200">
            <h2 className="text-lg font-semibold text-gray-800 mb-4">{t('My Policies')}</h2>
            <div className="space-y-3">
              <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                <div className="flex items-center gap-3">
                  <div className="p-2 rounded-full bg-blue-100"><Shield className="w-4 h-4 text-blue-600" /></div>
                  <div>
                    <p className="text-sm font-medium text-gray-800">{t('No active policies')}</p>
                    <p className="text-xs text-gray-400">{t('Purchase your first policy above')}</p>
                  </div>
                </div>
              </div>
            </div>
