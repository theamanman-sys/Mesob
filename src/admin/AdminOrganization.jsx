...
export default function AdminOrganization() {
  ...
  const [form, setForm] = useState({ name: '', description: '', icon: '', color: '', url: '', isActive: true })
  ...
  const handleSubmit = async (e) => {
    e.preventDefault()
    try { edit ? await organizationService.update(edit, form) : await organizationService.create(form); showToast(`Organization ${edit ? 'updated' : 'created'}`, 'success'); setModal(false); setEdit(null); setForm({ name: '', description: '', icon: '', color: '', url: '', isActive: true }); fetch() }
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center"><h1 className="text-2xl font-bold">Organizations</h1>
        <button onClick={() => { setEdit(null); setForm({ name: '', description: '', icon: '', color: '', url: '', isActive: true }); setModal(true) }} className="bg-blue-600 text-white px-4 py-2 rounded-md">Add Organization</button></div>
            ...
            {items.map((item) => (
              <tr key={item.id} className="hover:bg-gray-50">
                ...
                <td className="px-6 py-4 text-sm space-x-2">
                  <button onClick={() => { setForm({ name: item.name, description: item.description || '', icon: item.icon || '', color: item.color || '', url: item.url || '', isActive: item.isActive ?? true }); setEdit(item.id); setModal(true) }} className="text-blue-600 hover:text-blue-800">Edit</button>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white dark:bg-gray-800 rounded-lg p-6 w-full max-w-lg">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <input type="url" placeholder="Website URL" value={form.url} onChange={(e) => setForm({ ...form, url: e.target.value })} className="w-full px-3 py-2 border rounded-md" />
